...
analytics.get('/volume', async (c) => {
    ...
    let dateFilter = '';
    if (period === '7d')
        dateFilter = "AND w.start_time >= datetime('now', '-7 days')";
    else if (period === '30d')
        dateFilter = "AND w.start_time >= datetime('now', '-30 days')";
    else if (period === '90d')
        dateFilter = "AND w.start_time >= datetime('now', '-90 days')";
    else if (period === '1y')
        dateFilter = "AND w.start_time >= datetime('now', '-1 year')";
