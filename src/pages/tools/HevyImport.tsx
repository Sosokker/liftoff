...
export default function HevyImportPage() {
  ...
  return (
    <div class="px-4 py-6 space-y-6">
        ...
        <Show when={file()}>
          <div class="card p-4">
            ...
            <Show when={csvPreview()}>
              <div class="mb-4">
                <p class="text-xs font-medium text-neutral-400 mb-2 uppercase tracking-wider">
                  Preview (first 20 lines)
                </p>
                <div class="bg-[#1a1a1a] rounded-xl p-3 overflow-x-auto">
                  <pre class="text-xs text-neutral-300 whitespace-pre-wrap break-all">
                    {csvPreview().split('\n').slice(0, 20).join('\n')}
                  </pre>
                </div>
              </div>
      ...
      {/* Error */}
      <Show when={error()}>
        <div class="card p-4 bg-red-950/30 bg-red-900/20 border-red-200 border-red-800">
          <div class="flex items-start gap-3">
            <AlertCircle class="w-5 h-5 text-red-500 mt-0.5 flex-shrink-0" />
            <div>
              <p class="font-medium text-red-400 text-red-400">Import Failed</p>
              <p class="text-sm text-red-400 text-red-300 mt-1">{error()}</p>
            </div>
          </div>
        </div>
      </Show>
      ...
      <Show when={isImporting()}>
        <div class="card p-8 text-center">
          ...
          <Show when={progress().total > 0}>
            ...
            <div class="w-full bg-[#333] bg-[#333] rounded-full h-2 mt-3 overflow-hidden">
              <div
      ...
      <Show when={result()}>
        <div class="space-y-4">
          ...
          <Show when={result()?.errors && result()!.errors!.length > 0}>
            <div class="card p-4 bg-yellow-950/30 bg-yellow-900/20 border-yellow-200 border-yellow-800">
              <div class="flex items-start gap-3">
                <AlertCircle class="w-5 h-5 text-yellow-400 mt-0.5 flex-shrink-0" />
                <div>
                  <p class="font-medium text-yellow-400 text-yellow-400">
                    {result()!.errors!.length} warning{result()!.errors!.length > 1 ? 's' : ''}
                  </p>
                  <ul class="mt-2 space-y-1">
                    <For each={result()?.errors}>
                      {(err) => (
                        <li class="text-sm text-yellow-400 text-yellow-300">{err}</li>
                      )}
                    </For>
                  </ul>
                </div>
      ...
      <Show when={!file() && !result()}>
        <div class="card p-4">
          ...
          <ol class="space-y-2 text-sm text-neutral-300">
            <li class="flex gap-2">
